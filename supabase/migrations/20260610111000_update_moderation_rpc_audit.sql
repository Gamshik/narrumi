...
begin
  ...
  event_kind := case when next_warning_count >= 3 then 'ban' else 'warning' end;
  ...
  values (
    ...
    warning_reason,
    warning_excerpt,
    coalesce(ban_criteria -> 'evidence', '[]'::jsonb),
    event_details,
    now_value
  );
...
revoke all on function public.record_user_moderation_warning(text, text, jsonb, text, jsonb) from public;
grant execute on function public.record_user_moderation_warning(text, text, jsonb, text, jsonb) to authenticated;
