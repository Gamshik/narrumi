...
- Bundled Oxford usage examples received from the client remain untrusted model context, but they are excluded from user-strike moderation evidence because catalog prose is not learner-authored. Learner-authored setup, memory, choices, and replies remain subject to normal moderation.
