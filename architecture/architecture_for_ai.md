...
- Resolve dialogue ownership from the locally persisted series participation mode and canonical `userRole`. Render learner-character dialogue as a right-aligned outgoing bubble and other speakers as incoming bubbles; saved physical actions remain second-person narrative blocks.
- Render each learner answer exactly once. AI continuation frames must begin with consequences and must not reproduce the saved spoken answer as another incoming or generated dialogue bubble.
...
- Reader framing is semantic, not sentence-tokenization: group related narration sentences into meaningful paragraph or action-beat blocks, while keeping actual dialogue turns separate with pinned speaker metadata. The Writer must wrap direct speech in ASCII double quotation marks. Dialogue text contains all directly quoted words and only spoken words; speaker attribution, reported speech, character actions, body movement, facial expression, and stage direction remain narration. Deterministically downgrade every Utility dialogue frame whose wording cannot be found inside a quoted source span, including forms such as `Artem greets Gleb and pulls out a chair`. Apply the existing non-failing downgrade to attribution forms such as `Vlad says, leaning against the desk`. When a narration frame contains a pinned-speaker attribution followed by quoted speech, deterministically split it into narration plus a complete dialogue frame and remap annotations by their retained surface text. Deterministically remove a dialogue block of at least four words when it repeats the normalized tail of the immediately preceding narration block. The legacy `sentences` and `continuationSentences` field names carry these semantic blocks for mobile compatibility.
...
- Atomically claim generation scopes before model access. Return cached validated output for completed retries and `generation_in_progress` for the same active lease. The mobile client must persist the episode-slot request id before network access and remove it only after the validated response is saved locally. When that same durable episode request id returns after response loss, recover its cached completed output even if visible inputs changed meanwhile; synchronize local Episode Words from the recovered payload. Return `generation_conflict` for a genuinely new request id when a completed slot or still-active lease has different input. A failed or expired slot may atomically adopt a new request id and fingerprint so corrected inputs can retry without manual database cleanup.
