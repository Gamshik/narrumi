...
- **Local Store:** User-created series, creative briefs, setup provenance, independent setup drafts, episodes, series memory, selected word sets, learning signals, preferences, and sync metadata must be written locally first. Setup drafts are stored as an id-keyed collection: a fresh creation flow adds a new record, resuming updates that record, and finalizing or explicitly confirming discard deletes only its source record. Use a lightweight Expo-compatible local store: prefer `@react-native-async-storage/async-storage` for MVP key-value records; move to `expo-sqlite` only if structured querying, larger episode history, or complex local indexing becomes necessary.
...
- **Free-Reply Trust Boundary:** A learner-authored reply is moderated and analyzed by a dedicated Validator before creative generation. The Validator classifies clear English intent, produces optional structured correction, and reduces accepted input to one bounded story intent. Raw learner text never reaches Writer, Decision, Reviewer, repair, or memory prompts. Non-English, unclear, and off-topic input returns editable revision guidance without consuming the turn.
- **Interaction Idempotency:** The client persists one stable submission id with the pending answer before any connectivity or AI work. `submit-interaction` claims that id through the shared generation-request store, returns cached accepted output on retry, and prevents duplicate continuations across remounts or process restarts.
- **Moderation Attribution:** A user warning may be created only from new learner-authored text at the current trust boundary. Replayed setup, compact memory, summaries, generated prompts, generated choices, Oxford examples, and other AI-authored context are never strike evidence. Selecting a generated choice is not treated as authorship. Warning persistence is idempotent per stable setup-generation or episode-interaction submission identity, so transport retries cannot turn one policy decision into several strikes while a genuinely edited resubmission remains a new moderation event.
...
- **Controlled Context:** Edge Functions must send only the bounded context required by the action: participation mode and the visible idea, characters, learner identity, and title context for card-local setup generation; episode-level genre and CEFR, compact series memory, recent episode summary, selected Story Words with exact Oxford id, headword, part of speech, level, and at most two short examples from that entry, already encountered Story Word ids, safety constraints, and output schema for episode generation; or only the exact selected episode excerpt for translation, with no surrounding sentence or paragraph context. Hidden backward-compatible creative-brief values are preserved in storage but are not supplied by the four-card editor. Do not send unbounded full series history.
...
- **Series:** User-created story container with title, premise, participation mode, characters or user role, optional compatibility creative brief, per-field setup provenance, compact memory, and sync metadata. Creation and pre-first-episode editing expose only four cards: Role, Idea (stored as `premise`), Characters, and Title. New creative briefs use empty defaults and internal `fill-missing`; older creative-brief fields and strategy values remain readable and preserved for existing data without being exposed as controls. Legacy genre, CEFR, and tone columns remain readable and writable only for compatibility with older clients.
- **Episode:** Generated learning unit linked to a series with its selected CEFR level and genre, accumulated story text, semantic reader blocks stored under legacy sentence fields, several ordered interaction turns, local reply drafts, reply intent, stable submission identity, original learner replies, structured language feedback, an AI-controlled completion state, cliffhanger, Story Words, and summary update. Old episode rows without settings inherit the owning series values when read.
...
* **Structured AI Output:** Edge Functions must return structured, validated JSON for setup suggestions, episodes, interaction options, annotations, selected-text translations, corrections, and memory updates. A selected-text translation response contains only one bounded `translation` string. The mobile app must validate the response shape before rendering. Each create or pre-first-episode edit `Generate by AI` action sends a `fill-missing` request with an explicit generation target, receives a complete coherent setup, and applies only that target: premise, one to eight character profiles plus Character-mode role, or title. Earlier visible values remain immutable, and a non-character target must preserve an existing cast exactly. The server may continue accepting legacy `refine`/`rebuild` requests for compatibility, but those controls are not exposed by the mobile setup flow.
