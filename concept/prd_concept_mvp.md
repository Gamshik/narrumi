...
- Series setup text fields are required before a series can be saved as ready: title, premise, main characters, and user role for Character mode. The incomplete four-card form may still be saved locally as a draft from any card. Each fresh `New Series` flow owns a distinct local draft id, so saving another unfinished setup adds a draft instead of replacing an earlier one. Resuming and saving updates only the selected draft, and finalizing removes only the draft that produced that series. A learner may also swipe one draft row left and confirm deletion; this removes only that selected local snapshot and preserves every other draft.
...
- Opening an existing series reuses the same four-card Role, Idea, Characters, and Title setup flow. It is editable only while the series has no generated episodes and read-only after the first episode; backward-compatible hidden creative-brief values remain preserved without returning legacy controls to the UI.
...
- Episode preparation is one compact reversible two-step flow: `Scene` for CEFR and Genre, then `Story Words`. A two-node `Episode setup` surface shows the current step and, after `Scene`, compact Level and Genre summaries that reopen the first step without changing the current Story Words. The final action stays reachable in a fixed footer; while generation is active it is replaced in place by one calm `Writing your episode...` progress state. Do not add a review step, setup draft controls, or repeated full-card illustrations to this frequent flow.
